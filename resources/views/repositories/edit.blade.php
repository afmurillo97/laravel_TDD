    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-4">
                <form action="{{ route('repositories.update', $repository) }}" method="POST" class="max-w-full">
                    @csrf
                    @method('PUT')
                    <div class="mb-6">
                        <label for="url" class="block text-lg font-medium text-gray-700 mb-2">URL *</label>
                        <input type="text" name="url"  value="{{ $repository->url }}" class="form-input w-full rounded-md shadow-sm border border-gray-300 focus:ring focus:ring-blue-200 focus:border-blue-500" placeholder="Enter the repository URL">
                    </div>

                    <div class="mb-6">
                        <label for="description" class="block text-lg font-medium text-gray-700 mb-2">Description *</label>
                        <textarea name="description" class="form-input w-full rounded-md shadow-sm border border-gray-300 focus:ring focus:ring-blue-200 focus:border-blue-500" placeholder="Enter the repository description" rows="4">
                    ...
                    <div class="flex justify-end">
                        <button type="submit" class="inline-flex items-center px-6 py-3 bg-yellow-500 text-white font-bold rounded-md transition ease-in-out delay-75 hover:bg-yellow-600 hover:-translate-y-1 hover:scale-105">
